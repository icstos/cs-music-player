...
- **播放模式** — 顺序播放、单曲循环、随机播放
- **歌词显示** — 自动匹配 `lyrics/` 子目录中的 `.lrc` / `.txt` 歌词，LRC 支持时间轴高亮，当前播放行始终停在歌词区垂直中线上（歌词区尺寸变化时可见行数自适应）；歌词区右上角提供 **歌词同步** 与 **字体大小** 两个入口；播放时自动从 [LRCLIB](https://lrclib.net) 后台下载缺失歌词到 `lyrics/` 子目录
